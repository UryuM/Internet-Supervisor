document.addEventListener('DOMContentLoaded', () => {
    ...
    // 現在アクティブなタブのドメインを追加するボタンを表示（拡張機能）
      ...
      if (tabs.length > 0 && tabs[0].url) {
        try {
          ...
          if (domain && domain !== "") {
            ...
            quickAddDiv.className = 'info-box';
            quickAddDiv.style.backgroundColor = '#e6f4ea';
            quickAddDiv.style.marginTop = '16px';
            
            const infoIcon = document.createElement('span');
            infoIcon.className = 'material-icons';
            infoIcon.textContent = 'add_circle';
            infoIcon.style.color = 'var(--success-color)';
            
            const infoText = document.createElement('div');
            infoText.style.flex = '1';
            
            const infoP = document.createElement('p');
            infoP.innerHTML = `現在開いているサイト <strong>${domain}</strong> をブロックリストに追加しますか？`;
            
            const quickAddBtn = document.createElement('button');
            quickAddBtn.className = 'btn-primary';
            quickAddBtn.style.marginTop = '8px';
            quickAddBtn.innerHTML = '<span class="material-icons">add</span> 現在のサイトを追加';
            
            quickAddBtn.addEventListener('click', () => {
              // inputに値を入れてaddDomainボタンをクリック
            ...
            infoText.appendChild(quickAddBtn);
            quickAddDiv.appendChild(infoIcon);
            quickAddDiv.appendChild(infoText);
            
            // カードの先頭に挿入
            const inputGroup = document.querySelector('.input-group');
            inputGroup.parentNode.insertBefore(quickAddDiv, inputGroup.nextSibling);
          }
        } catch (e) {
